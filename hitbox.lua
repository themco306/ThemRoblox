...
rightFrame.CanvasSize = UDim2.new(0, 0, 0, 0)  -- Đặt kích thước cuộn ban đầu
rightFrame.ScrollBarThickness = 12  -- Độ dày của thanh cuộn
...
local function createIslandButtons()
    local buttonWidth = 150  -- Kích thước chiều rộng của mỗi nút
    local buttonHeight = 40  -- Kích thước chiều cao của mỗi nút
    local buttonsPerRow = 3  -- Số nút mỗi hàng
    local padding = 10  -- Khoảng cách giữa các nút
    local startPositionX = 0  -- Vị trí X bắt đầu của nút đầu tiên
    local startPositionY = 0  -- Vị trí Y bắt đầu của nút đầu tiên

    -- Cập nhật CanvasSize của rightFrame mỗi khi tạo nút mới
    local totalButtonsHeight = math.ceil(#SecondSea / buttonsPerRow) * (buttonHeight + padding)
    rightFrame.CanvasSize = UDim2.new(0, 0, 0, totalButtonsHeight)  -- Điều chỉnh kích thước cuộn

    for i, island in ipairs(SecondSea) do
        -- Tạo nút cho mỗi đảo
        local teleportButton = Instance.new("TextButton")
        teleportButton.Size = UDim2.new(0, buttonWidth, 0, buttonHeight)  -- Kích thước nút
        ...
        -- Tính toán vị trí của mỗi nút
        local column = (i - 1) % buttonsPerRow
        local row = math.floor((i - 1) / buttonsPerRow)
        
        teleportButton.Position = UDim2.new(0, startPositionX + column * (buttonWidth + padding), 0, startPositionY + row * (buttonHeight + padding))
